...
const char* find_ci(const char *path, const char *filename)
{
     DIR *dir;
     struct dirent *entry;
     ...
     {
          if (strncasecmp(filename,entry->d_name,PATH_MAX)==0)
          {
               const char *n=strdup(entry->d_name); 
               closedir(dir);
               return n; 
          }
     }
...
int extractFirmware(const char *model)
{
     ...
     printf("Searching %s\n", filename);
     const char *ifilename=find_ci(".",filename);
     printf("Found %s\n", ifilename);


     bfd_init();
     file = bfd_openr(ifilename, "efi-app-ia32");
     if (!file)
          file = bfd_openr(ifilename, "pei-i386");
     if (!file)
     {
